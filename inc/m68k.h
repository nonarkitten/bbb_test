...
//#define HASH_MAP_SIZE 59999
//
//typedef union {
//    struct {
//        uint16_t* address;  // original M68K memory location
//        uint32_t checksum; // complete crc of all words
//        uint16_t reject;  // copy of the first 32-bits at this
//        uint16_t length;  // number of M68K ops we read
//
//        // Start of the ARM code to execute; we make this a
//        // union so that we can write the data here and call
//        // it later without an ISO warning cast
//        union {
//            void*(*caller)(void*);
//            uint32_t code[0];
//        };
//    };
//    
//    // Make the block size a fixed length; this allows us to
//    // statically allocate while having a "flexible" member
//    uint8_t raw[BLOCK_SIZE];
//} code_block_t;

//typedef void(*mt_Callback)(void**args);
//
struct M68KTranslationUnit {
    ...
    union {
    uint32_t        mt_ARMCode[1];
    void          (*mt_Callback) (void*args);
    };
};
...
extern void          M68K_PushReturnAddress(uint16_t *ret_addr);
extern uint16_t*     M68K_PopReturnAddress(uint8_t *success);
extern void          M68K_ResetReturnStack();
                     
extern uint8_t       M68K_GetSRMask(uint16_t *m68k_stream);
extern void          M68K_InitializeCache();
extern void          M68K_DumpStats();
extern uint8_t       M68K_GetCC(uint32_t **ptr);
extern uint8_t       M68K_ModifyCC(uint32_t **ptr);
extern void          M68K_FlushCC(uint32_t **ptr);

extern struct M68KTranslationUnit* 
    M68K_GetTranslationUnit(uint16_t *ptr);
